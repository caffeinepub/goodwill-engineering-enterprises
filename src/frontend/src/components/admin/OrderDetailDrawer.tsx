...
import { useUpdateOrderStatus, useGetAllProducts } from '../../hooks/useQueries';
...
export function OrderDetailDrawer({ order, open, onOpenChange }: OrderDetailDrawerProps) {
  ...
  const handleExportJSON = () => {
    const data = JSON.stringify(order, (_, v) => typeof v === 'bigint' ? v.toString() : v, 2);
              ...
              {order.cartItems.map((item, index) => {
                ...
                return (
                  <div key={index} className="p-3 bg-muted rounded-lg space-y-1">
                    ...
                    {item.customNotes && (
                      <p className="text-sm text-muted-foreground">Notes: {item.customNotes}</p>
                    )}
                  </div>
                );
              })}
