...
export function useUploadProductImage() {
  ...
  return useMutation({
    mutationFn: async ({
      productId,
      externalBlob,
    }: { productId: bigint; externalBlob: ExternalBlob }) => {
      if (!actor) throw new Error("Actor not available");
...
export function useAddToCart() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      quantity,
      customNotes,
    }: { productId: bigint; quantity: bigint; customNotes?: string }) => {
      if (!actor) throw new Error("Actor not available");
      return actor.addToCart(productId, quantity, customNotes || null);
    },
    onSuccess: () => {
...
export function useSubmitWhatsAppOrder() {
  ...
  return useMutation({
    mutationFn: async ({
      cartItems,
      whatsappNumber,
    }: { cartItems: CartItem[]; whatsappNumber: string }) => {
      if (!actor) throw new Error("Actor not available");
